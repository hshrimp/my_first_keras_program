...
def train(num_words,embedding_matrix,x1_train,x2_train,y_train,x1_val,x2_val,y_val):
    ...
    x1=Conv1D(200,3,activation='relu',kernel_regularizer=regularizers.l2(0.01))(x1)
    ...
    model.fit([x1_train,x2_train],y_train,batch_size=200,epochs=600, verbose=2,callbacks=[EarlyStopping(monitor='val_loss',min_delta=0.0001,patience=5)],validation_data=([x1_val,x2_val],y_val))
    #plot_model(shall_model, to_file='model_1.png', show_shapes=True)
    plot_model(model,to_file='new_a14.png',show_shapes=True)
