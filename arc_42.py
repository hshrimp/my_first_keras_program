...
from keras.layers import Conv1D,Conv2D,MaxPooling2D,Embedding,merge
...
def train(num_words, embedding_matrix,  x1_train,x2_train, y_train, x1_val,x2_val, y_val):
    ...
    attention = Activation(activation='softmax',name='act_layer')(x)
    ...
    model.fit([x1_train,x2_train],y_train,batch_size=100,epochs=200,verbose=2,callbacks=[EarlyStopping(monitor='val_loss',min_delta=0.0001,patience=2)],validation_data=([x1_val,x2_val],y_val))
    # plot_model(shall_model, to_file='model_1.png', show_shapes=True)
    plot_model(model,to_file='ARC_42.png',show_shapes=True)

    # # 取某一层的输出为输出新建为model，采用函数模型
    # attention_model = Model(inputs=model.input,
    #                            outputs=model.get_layer('act_layer').output)
    # # 以这个model的预测值作为输出
    # dense1_output = attention_model.predict([x1_val,x2_val])
    #
