...
def train(num_words,embedding_matrix,x1_train,x2_train,y_train,x1_val,x2_val,y_val):
    ...
    x1=Conv1D(500,3,activation='relu')(embedded_sequences1)
    x1 = Conv1D(500, 3, activation='relu')(x1)
    x1 = Conv1D(500, 3, activation='relu')(x1)
    ...
    model.fit([x1_train,x2_train],y_train,batch_size=200,epochs=500, verbose=2,callbacks=[EarlyStopping(monitor='val_loss',min_delta=0.0001,patience=3)],validation_data=([x1_val,x2_val],y_val))
    #plot_model(shall_model, to_file='model_1.png', show_shapes=True)
    plot_model(model,to_file='a124.png',show_shapes=True)
