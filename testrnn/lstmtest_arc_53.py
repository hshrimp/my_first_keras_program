...
from keras.layers import Conv1D,Conv2D,MaxPooling2D,Embedding,LSTM,Masking
...
def train(num_words, embedding_matrix,  x1_train,x2_train, y_train, x1_val,x2_val, y_val):
    ...
    embedding_layer=Embedding(num_words,embedding_dim,weights=[embedding_matrix],mask_zero=True,input_length=max_sequence_length,trainable=False)
    print('training model.')

    sequence_input=Input(shape=(max_sequence_length,),dtype='int32')

    embedded_sequences=embedding_layer(sequence_input)
    x=LSTM(embedding_dim,return_sequences=True,dropout=0.2, recurrent_dropout=0.2)(embedded_sequences)
    ...
    Embedding_model=Model(sequence_input,x)
    ...
    model.fit([x1_train,x2_train],y_train,batch_size=100,epochs=100,callbacks=[EarlyStopping(monitor='val_loss',min_delta=0.0001,patience=2)],validation_data=([x1_val,x2_val],y_val))
    # plot_model(shall_model, to_file='model_1.png', show_shapes=True)
    plot_model(model,to_file='lstmbacktestARC_53.png',show_shapes=True)
