...
from keras.layers import Multiply,Conv2D,MaxPooling2D,Embedding,merge,LSTM
...
def train(num_words, embedding_matrix,  x1_train,x2_train, y_train, x1_val,x2_val, y_val):
    ...
    x=dot([embedded_sequences1,embedded_sequences2],axes=2)
    attention=Activation(activation='softmax')(x)
    x=merge([x,attention],mode='mul')
    x=Reshape((19,19,1))(x)
    x=Conv2D(200,(2,2),activation='relu',kernel_regularizer=regularizers.l2(0.01))(x)
    #x=Reshape((18,18,200))(x)
    ...
    model.fit([x1_train,x2_train],y_train,batch_size=100,epochs=200,callbacks=[EarlyStopping(monitor='val_loss',min_delta=0.0001,patience=1)],validation_data=([x1_val,x2_val],y_val))
    # plot_model(shall_model, to_file='model_1.png', show_shapes=True)
    plot_model(model,to_file='ARC_72.png',show_shapes=True)
