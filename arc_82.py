...
from keras.layers import Dense,Dropout,Input,Flatten,Lambda,Reshape,Activation
from keras.layers import Conv1D,Conv2D,MaxPooling2D,Embedding,merge,LSTM
...
def train(num_words, embedding_matrix,  x1_train,x2_train, y_train, x1_val,x2_val, y_val):
    ...
    embedded_sequences1 = LSTM(embedding_dim, return_sequences=True)(embedded_sequences1)
    embedded_sequences1 = LSTM(embedding_dim, return_sequences=True, go_backwards=True)(embedded_sequences1)
    embedded_sequences2 = LSTM(embedding_dim, return_sequences=True)(embedded_sequences2)
    embedded_sequences2 = LSTM(embedding_dim, return_sequences=True, go_backwards=True)(embedded_sequences2)
    x1=dot([embedded_sequences1,embedded_sequences2],axes=2)
    attention = Activation(activation='softmax')(x1)
    x1 = merge([x1, attention], mode='mul')
    x2 = dot([embedded_sequences2, embedded_sequences1], axes=2)
    attention = Activation(activation='softmax')(x2)
    x2 = merge([x2, attention], mode='mul')
    x = concatenate([x1, x2],axis=1)
    #x=Reshape((19,19,1))(x)
    x=Conv1D(200,3,activation='relu',kernel_regularizer=regularizers.l2(0.01))(x)
    x=Reshape((6,6,200))(x)
    x=MaxPooling2D((2,2),strides=2)(x)
    #x1=Dropout(0.5)(x1)
    x=Conv2D(200,(3,3),activation='relu',kernel_regularizer=regularizers.l2(0.01))(x)
    # x = MaxPooling2D((2,2),strides=2)(x)
    # x = Conv2D(200, (2, 2),strides=2, activation='relu', kernel_regularizer=regularizers.l2(0.01))(x)
    # x = MaxPooling2D((2, 2), strides=2)(x)
    #x1 = Dropout(0.5)(x1)
    ...
    model.fit([x1_train,x2_train],y_train,batch_size=100,epochs=200,verbose=2,callbacks=[EarlyStopping(monitor='val_loss',min_delta=0.0001,patience=10)],validation_data=([x1_val,x2_val],y_val))
    # plot_model(shall_model, to_file='model_1.png', show_shapes=True)
    plot_model(model,to_file='ARC_82.png',show_shapes=True)
