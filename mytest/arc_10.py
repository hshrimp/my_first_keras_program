...
def train(num_words,embedding_matrix,x_train,y_train,x_val,y_val):
    ...
    model.fit(x_train,y_train,batch_size=100,epochs=500, verbose=2,validation_data=(x_val,y_val),callbacks=[EarlyStopping(monitor='val_loss',min_delta=0.0001,patience=2)] )
    #plot_model(shall_model, to_file='model_1.png', show_shapes=True)
    plot_model(model,to_file='ARC_10.png',show_shapes=True)

    yp = model.predict(x_val, batch_size=100, verbose=1)
