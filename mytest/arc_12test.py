...
def train(num_words,embedding_matrix,x1_train,x2_train,y_train,x1_val,x2_val,y_val):
    ...
    model.fit([x1_train,x2_train],y_train,batch_size=100,epochs=500,callbacks=[EarlyStopping(monitor='val_loss',min_delta=0.0001,patience=2)],validation_data=([x1_val,x2_val],y_val))
    plot_model(shall_model, to_file='model_1.png', show_shapes=True)
    plot_model(model,to_file='ARC_12test.png',show_shapes=True)
