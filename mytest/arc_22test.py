...
from keras.layers import Conv1D,Conv2D,MaxPooling2D,Embedding,AveragePooling2D
...
max_sequence_length=6534 #(35-2)(35-2)*6=6534,句子最长35
max_nb_words=20000
embedding_dim=50
...
def to_tensor(texts11,texts12,labels1,texts21,texts22,labels2):
    ...
    #text11的张量
    data1 = pad_sequences(sequences1,max_sequence_length)
    #text12的张量
    data2 = pad_sequences(sequences2, max_sequence_length)
    ...
    print('shape of the data11 tensor:',data1.shape)
    #print('shape of the data12 tensor:', data12.shape)
    print('shape of label1 tensor:',labels1.shape)
    print('shape of the data21 tensor:', data2.shape)
    #print('shape of the data22 tensor:', data22.shape)
    print('shape of label2 tensor:', labels2.shape)
    return word_index, data1,labels1,data2,labels2

#生成交互序列，window=3
def to_interact(sequences1,sequences2):
    interact_matric=[]
    for sen_num in range(len(sequences1)):
        sen1_len = len(sequences1[sen_num])
        sen2_len = len(sequences2[sen_num])
        seg = []
        for i in range(sen1_len-2):
            seg1 = sequences1[sen_num][i:i + 3]
            for j in range(sen2_len-2):
                seg2=sequences2[sen_num][j:j+3]
                # seg.append(seg1)
                # seg.append(seg2)
                seg = np.append(seg, seg1)
                seg = np.append(seg, seg2)
        #seg = [seg]
        interact_matric.append(seg.astype(int))
    return interact_matric
...
def train(num_words,embedding_matrix,x_train,y_train,x_val,y_val):
    ...
    x=Conv1D(200,6,strides=6,activation='relu',kernel_regularizer=regularizers.l2(0.01))(embedded_sequences)
    x=Reshape((33,33,200))(x)
    #x=AveragePooling2D((2,2),strides=2)(x)
    #x1=Dropout(0.5)(x1)
    x=Conv2D(200,(3,3),activation='relu',kernel_regularizer=regularizers.l2(0.01))(x)
    #x = AveragePooling2D((2,2),strides=2)(x)
    x = Conv2D(200, (6, 6), activation='relu', kernel_regularizer=regularizers.l2(0.01))(x)
    #x = AveragePooling2D((2, 2), strides=2)(x)
    #x1 = Dropout(0.5)(x1)
    ...
    model.fit(x_train,y_train,batch_size=200,epochs=500, verbose=2,validation_data=(x_val,y_val),callbacks=[EarlyStopping(monitor='val_loss',min_delta=0.0001,patience=2)] )
    #plot_model(shall_model, to_file='model_1.png', show_shapes=True)
    plot_model(model,to_file='ARC_22test.png',show_shapes=True)

    yp = model.predict(x_val, batch_size=200, verbose=1)
...
if __name__ == '__main__':
    ...
    word_index, x_train, y_train, x_val, y_val=to_tensor(x1text_train,x2text_train,train_label,x1text_test,x2text_test,test_label)
    num_words, embedding_matrix=prepare_embedding(word_index,embeddings_index)
    train(num_words, embedding_matrix,  x_train, y_train, x_val, y_val)
