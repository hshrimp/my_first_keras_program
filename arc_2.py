...
from keras.layers import Conv1D,Conv2D,MaxPooling2D,Embedding
...
def train(num_words,embedding_matrix,x_train,y_train,x_val,y_val):
    ...
    x=Conv1D(200,6,strides=6,activation='relu')(embedded_sequences)
    x=Reshape((33,33,200))(x)
    x=MaxPooling2D((2,2),strides=2)(x)
    #x1=Dropout(0.5)(x1)
    x=Conv2D(200,(3,3),activation='relu')(x)
    x = MaxPooling2D((2,2),strides=2)(x)
    x = Conv2D(200, (6, 6), activation='relu')(x)
    x = MaxPooling2D((2, 2), strides=2)(x)
    #x1 = Dropout(0.5)(x1)
    ...
    model.fit(x_train,y_train,batch_size=200,epochs=400,validation_data=(x_val,y_val),callbacks=[EarlyStopping(monitor='val_loss',min_delta=0.0001,patience=10)] )
    # plot_model(shall_model, to_file='model_1.png', show_shapes=True)
    #plot_model(model,to_file='ARC_2.png',show_shapes=True)
