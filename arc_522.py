...
from keras.layers import Conv1D,Conv2D,MaxPooling2D,Embedding,LSTM,Bidirectional
...
def train(num_words, embedding_matrix,  x1_train,x2_train, y_train, x1_val,x2_val, y_val):
    ...
    lstm_right=LSTM(embedding_dim,return_sequences=True)(embedded_sequences)
    lstm_right=Reshape((20,50,))(lstm_right)
    lstm_left=LSTM(embedding_dim,return_sequences=True,go_backwards=True)(lstm_right)
    #x = Bidirectional(LSTM(embedding_dim, return_sequences=True),merge_mode='sum', name='BiLSTM')(embedded_sequences)
    output=Reshape((20,50,))(lstm_left)
    ...
    x = MaxPooling2D((2,2),strides=2)(x)
    x = Conv2D(200, (2, 2),strides=2, activation='relu')(x)
    x = MaxPooling2D((2, 2), strides=2)(x)
    #x1 = Dropout(0.5)(x1)
    ...
    plot_model(model,to_file='ARC_522.png',show_shapes=True)
